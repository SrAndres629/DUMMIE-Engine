...
EVIDENCE_JSON = os.path.join(
    AIWG_DIR, "reports", "pack_validation_evidence_latest.json"
)
...
def check_anti_overclaim_lint():
    ...
    generated_log_files = {
        os.path.join("state", "health_cron_latest.txt"),
    }
    allowlist = [
    ...
    for subdir in target_subdirs:
        ...
        for root, _, files in os.walk(dir_path):
            for file in files:
                ...
                if os.path.relpath(file_path, AIWG_DIR) in generated_log_files:
                    continue
                try:
    ...
    if found_violations:
        print(
            "ERROR: Anti-overclaim lint failed. The following unallowlisted overclaims were detected in .aiwg/:"
        )
        for v in found_violations:
            print(
                f"  - File: {v['file']}:{v['line']} (found term '{v['term']}'): '{v['content']}'"
            )
        return False
...
def run_preflight(args):
    ...
    if truth_head != git_head and not is_symbolic_head(truth_head):
        print(
            f"ERROR: current_truth.json head_commit ({truth_head}) is stale. Current actual HEAD: {git_head}"
        )
        sys.exit(1)

    if is_symbolic_head(truth_head):
        print(
            f"WARNING: current_truth.head_commit is symbolic ({truth_head}); runtime HEAD ({git_head}) will be used for this validation."
        )
    ...
    if os.path.exists(ROADMAP_JSON):
        ...
        if roadmap:
            ...
            if roadmap_head != git_head and not is_symbolic_head(roadmap_head):
                print(
                    f"ERROR: pack_roadmap_to_6_1.json head_commit ({roadmap_head}) is stale. Current actual HEAD: {git_head}"
                )
                sys.exit(1)
            if is_symbolic_head(roadmap_head):
                print(
                    f"WARNING: pack_roadmap_to_6_1.json head_commit is symbolic ({roadmap_head}); runtime HEAD ({git_head}) will be used for this validation."
                )
        ...
        if os.path.exists(path) and path.endswith(".json"):
            ...
            if rep_data and isinstance(rep_data, dict):
                ...
                if rgc and rgc != git_head and rgc != "UNVERIFIED":
                    print(
                        f"ERROR: Report '{name}' generated commit ({rgc}) is stale. Current actual HEAD: {git_head}"
                    )
                    sys.exit(1)
    ...
    if (
        ...
        or len(rollback.strip()) == 0
        or rollback.strip() == "UNVERIFIED"
    ):
    ...
    if pack_id == "PACK_3.2" and last_completed != "PACK_3.1":
        ...
        if not roadmap_completed:
            print(
                f"ERROR: Cannot start {pack_id} while last_completed_pack is {last_completed} (expected PACK_3.1)."
            )
            sys.exit(1)
    ...
    for name, path in reports.items():
        if not os.path.exists(path):
            print(
                f"ERROR: Report drift detected. Registered report '{name}' is missing at: {path}"
            )
            sys.exit(1)
...
def validate_critique_field(field_name, val):
    ...
    for pattern in optimistic_patterns:
        if re.search(pattern, val_strip, re.IGNORECASE):
            print(
                f"WARNING: Rejected optimistic default value '{val_strip}' for field '{field_name}'"
            )
            return "UNVERIFIED"
...
def run_self_critique(args):
    ...
    answers = {
        ...
        "advances_degraded": validate_critique_field(
            "advances_degraded", args.degraded
        ),
...
"""
    ...
    print(
        f"SUCCESS: Generated self-critique reports under {CRITIQUE_JSON}, {CRITIQUE_MD} and archive {archive_path}"
    )
...
def run_run_required(args):
    ...
    if not commands:
        print(
            "ERROR: No commands_required or tests_required found in active_pack.json."
        )
        sys.exit(1)
    ...
    for cmd in commands:
        if "python3" in cmd and re.search(r"scripts/[^ ]*semantic[^ ]*", cmd):
            print(
                f"ERROR: Command '{cmd}' uses 'python3' directly for a critical semantic index script."
            )
            print(
                "You must use 'PYTHONPATH=. layers/l2_brain/.venv/bin/python' instead."
            )
            sys.exit(1)
...
def run_closeout(args):
    ...
    for k, v in critique.get("answers", {}).items():
        if v == "UNVERIFIED" or not v:
            print(
                f"ERROR: Self-critique answer for '{k}' is UNVERIFIED. Cannot close out."
            )
            sys.exit(1)
    ...
    if (
        not evidence.get("run_by_runner")
        or "stdout_log_path" not in evidence
        or "duration_seconds" not in evidence
    ):
        print(
            "ERROR: Closeout failed. Validation evidence has an invalid or manually generated schema."
        )
        sys.exit(1)
    ...
    if evidence.get("exit_code") != 0:
        print(
            f"ERROR: Closeout failed. Stored validation evidence shows exit_code = {evidence.get('exit_code')}"
        )
        sys.exit(1)
    ...
    if evidence.get("commit") != git_head:
        print(
            f"ERROR: Validation evidence is stale. Evidence commit: {evidence.get('commit')}, actual HEAD: {git_head}"
        )
        sys.exit(1)
    ...
    if "pytest" not in cmd_str:
        print(
            "ERROR: Closeout failed. pytest execution is missing from validation evidence."
        )
        sys.exit(1)

    if "validate_specs_docs" not in cmd_str:
        print(
            "ERROR: Closeout failed. validate_specs_docs execution is missing from validation evidence."
        )
        sys.exit(1)
    ...
    if truth_head != git_head and not is_symbolic_head(truth_head):
        print(
            f"ERROR: current_truth.json is stale. Current HEAD: {git_head}, current_truth: {truth_head}"
        )
    ...
    if is_symbolic_head(truth_head):
        ...
        if policy != "versioned_snapshot_self_reference":
            print(
                f"ERROR: current_truth.json has symbolic head_commit ({truth_head}) but invalid policy ({policy})."
            )
            sys.exit(1)
        print(
            f"WARNING: current_truth.head_commit is symbolic ({truth_head}); policy '{policy}' is active."
        )
    ...
    if is_symbolic_head(roadmap_head):
        print(
            f"WARNING: pack_roadmap_to_6_1.json head_commit is symbolic ({roadmap_head})."
        )
    ...
    if truth.get("current_pack") != pack_id:
        print(
            f"ERROR: Closeout mismatch. current_truth.json represents current_pack={truth.get('current_pack')}, expected {pack_id}."
        )
        sys.exit(1)
    ...
    for pack in roadmap.get("packs", []):
        ...
        if sha and sha != "UNVERIFIED":
            if not re.match(r"^[a-f0-9]{7,40}$", sha):
                print(
                    f"ERROR: Invalid source_of_truth_commit '{sha}' in roadmap for pack '{pack.get('pack_id')}'"
                )
                sys.exit(1)
    ...
    for k, v in dist.items():
        if k.endswith("_status") or k.endswith("_state"):
            if v not in allowed_states:
                print(
                    f"ERROR: Distance metric state '{v}' for '{k}' is invalid (must be measured, estimated, or unverified)."
                )
                sys.exit(1)

    current_score = dist.get("current_score", 0.0)
    print(
        f"Current completion score towards Pack 6.1 (Golden Path): {current_score:.2f} (Scale 0.0 to 1.0)"
    )
...
def run_next_pack(args):
    ...
    if next_pack == "PACK_3.2" and last_completed != "PACK_3.1":
        ...
        if not roadmap_completed:
            print(
                "ERROR: Anti-skip gate active. PACK_3.1 must be closed in main first."
            )
            sys.exit(1)
...
def main():
    ...
    subparsers.add_parser(
        "preflight", help="Validate active pack contract preconditions"
    )

    # Self-critique
    sc_parser = subparsers.add_parser(
        "self-critique", help="Generate self-critique answers"
    )
    ...
    sc_parser.add_argument(
        "--goal", help="How does this pack move towards Golden Path 6.1?"
    )

    # Record Evidence
    re_parser = subparsers.add_parser(
        "record-evidence", help="Record test validation evidence"
    )
    ...
    re_parser.add_argument(
        "--exit-code", required=True, help="Exit code of the execution"
    )
    ...
    # Distance
    subparsers.add_parser(
        "distance", help="Measure project distance metrics towards Pack 6.1"
    )

    # Next Pack
    subparsers.add_parser(
        "next-pack", help="Report next scheduled pack and check skip violations"
    )

    # Run Required
    subparsers.add_parser(
        "run-required", help="Run required commands, capture real evidence and logs"
    )
