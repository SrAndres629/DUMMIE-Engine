...
def build_initial_consensus(pack):
    return {"pack": pack, "worker_vote": True, "reviewer_vote": None, "deadlock": False}


def apply_worker_ack(consensus):
    consensus["worker_vote"] = True
    if consensus.get("reviewer_vote") is not True:
        consensus["reviewer_vote"] = None
    consensus["deadlock"] = False
    return consensus
...
def main():
    ...
    consensus = build_initial_consensus(args.pack)
    if cons_file.exists():
        try:
            existing = json.loads(cons_file.read_text())
            consensus = apply_worker_ack(existing)
        except:
            pass
