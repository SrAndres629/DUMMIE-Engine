...
class TuiProcessMonitor:
    ...
    def build_process_monitor_snapshot(self, write_output: bool = True) -> ProcessMonitorSnapshot:
        warnings: list[str] = []
        evidence_refs: list[str] = []

        current = self._load_json(self.aiwg_root / "evolution" / "current_position.json", warnings, evidence_refs)
        seed = self._load_json(self.aiwg_root / "evolution" / "next_phase_seed.json", warnings, evidence_refs)
        restart = self._load_json(self.reports_root / "restart_integration_gate_latest.json", warnings, evidence_refs)
        bench = self._load_json(self.reports_root / "context_efficiency_benchmark_latest.json", warnings, evidence_refs)
        fly = self._load_json(self.reports_root / "evolution_flywheel_latest.json", warnings, evidence_refs)
        cache = self._load_json(self.reports_root / "prompt_cache_summary_latest.json", warnings, evidence_refs)
        stale = self._load_json(self.reports_root / "stale_memory_report.json", warnings, evidence_refs)
        quant = self._load_json(self.reports_root / "context_quant_result_latest.json", warnings, evidence_refs)
        ...
        snapshot = ProcessMonitorSnapshot(
            ...
            decision=decision,
            warnings=warnings,
            evidence_refs=evidence_refs + [".aiwg/reports/context_quant_result_latest.json"],
...
def build_process_monitor_snapshot(aiwg_root: str | Path = ".aiwg", write_output: bool = True) -> ProcessMonitorSnapshot:
    return TuiProcessMonitor(aiwg_root=aiwg_root).build_process_monitor_snapshot(write_output=write_output)
