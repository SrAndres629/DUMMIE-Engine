...
class MentalModelStore:
    ...
    def append_model(self, model):
        ...
        index[model.model_id] = {
            "path": str(self.jsonl_path.relative_to(self.root.parent.parent))
            if self.root.parent.parent != Path(".")
            else str(self.jsonl_path),
            "status": "valid",
    ...
    def mark_status(
        self, model_id: str, status: str, reason: str, superseded_by: str = ""
    ):
        ...
        entry.setdefault("hygiene_findings", []).append(
    ...
    def find_best_model_for_intent(self, intent: str):
        ...
        for mid, entry in index.items():
            ...
            if entry.get("status") in ("quarantined", "unsafe_rejected", "superseded"):
                continue
            if (
                entry.get("intent_hash") == target_hash
                and entry.get("quality_score", -1) > best_score
            ):
