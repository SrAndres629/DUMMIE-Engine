...
_INTENT_PATTERNS = {
    "CREATE": re.compile(
        r"\b(crea|create|new|genera|build|implement|add)\b", re.IGNORECASE
    ),
    "FIX": re.compile(
        r"\b(fix|repair|corrige|arregla|debug|solve|resolve)\b", re.IGNORECASE
    ),
    "REFACTOR": re.compile(
        r"\b(refactor\w*|reorgan|restructur|clean|simplif|optimiz)\b", re.IGNORECASE
    ),
    "ANALYZE": re.compile(
        r"\b(analy|audit|review|inspect|evalua|diagnos|investig)\b", re.IGNORECASE
    ),
    "DELETE": re.compile(r"\b(delete|remove|elimina|deprecat|drop)\b", re.IGNORECASE),
    "QUERY": re.compile(
        r"\b(show|list|status|what|where|how|why|explain|describe)\b", re.IGNORECASE
    ),
}

_LANGUAGE_PATTERNS = {
    "es": re.compile(
        r"\b(que|como|para|desde|hacer|quiero|necesito|debería|además|donde)\b",
        re.IGNORECASE,
    ),
    "en": re.compile(
        r"\b(should|would|could|please|want|need|where|which|implement)\b",
        re.IGNORECASE,
    ),
}
...
_LAYER_REFS = {
    "l0": re.compile(
        r"\b(l0|overseer|go\.mod|elixir|mix\.exs|dummied)\b", re.IGNORECASE
    ),
    "l1": re.compile(
        r"\b(l1|nervous|mcp_server|mcp_proxy|tools\.py|gateway)\b", re.IGNORECASE
    ),
    "l2": re.compile(
        r"\b(l2|brain|daemon|orchestrat|skill|planner|memory|model)\b", re.IGNORECASE
    ),
...
def _build_suborders(intent: str, layer_refs: list[str], language: str) -> list[str]:
    ...
    if len(layer_refs) > 2:
        suborders.append(
            "This is a cross-layer change. Verify contracts between layers."
        )
...
def preprocess_with_llm(
    ...
    base_url = (
        base_url or os.getenv("DUMMIE_OLLAMA_BASE_URL", "http://127.0.0.1:11434")
    ).rstrip("/")
    ...
    try:
        ...
        return PreprocessingResult(
            ...
            enriched_prompt="\n".join(enriched_parts),
            extracted_intent=intent
            if intent in {i for i in _INTENT_PATTERNS}
            else baseline.extracted_intent,
        ...
        logger.warning(
            "LLM preprocessing unavailable (%s), using deterministic fallback", exc
        )
...
class PromptPreprocessor:
    ...
    def __init__(self, use_llm: bool | None = None, pre_input_hook: Any = None):
        if use_llm is None:
            self.use_llm = os.getenv("DUMMIE_PREPROCESS_LLM", "1").strip().lower() in {
                "1",
                "true",
                "yes",
            }
    ...
    def process(self, prompt: str) -> PreprocessingResult:
        ...
        result.hook_metadata = _run_optional_pre_input_hook(
            self.pre_input_hook, prompt, result
        )
        return result
...
def _run_optional_pre_input_hook(
    ...
    try:
        try:
            ...
            from layers.l2_brain.cognitive_hooks import (
                CognitiveHookInput,
                CognitiveHookPacket,
            )
        ...
        logger.warning(
            "Pre-input hook unavailable (%s), preserving preprocessing fallback", exc
        )
