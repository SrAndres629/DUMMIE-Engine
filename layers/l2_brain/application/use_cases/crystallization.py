...
from layers.l2_brain.domain.memory.models import MemoryNode4DTES, CrystallizedSkill
...
class CrystallizeProceduralMemoryUseCase:
    ...
    def execute(
        self, context: SixDimensionalContext, source_nodes: List[MemoryNode4DTES]
    ) -> CrystallizedSkill:
        """
        ...
        certainty_score = (
            stats.tests_passing / total_signals if total_signals > 0 else 0.0
        )

        if certainty_score < 0.85:  # Umbral definido en Spec 38
            raise ValueError(
                f"Certeza insuficiente ({certainty_score:.2f}) para cristalización en {context.locus_x}"
            )
...
"""
        ...
        decision = DecisionRecord(
            ...
            witness_hash=hashlib.sha256(skill_hash.encode()).hexdigest()[
                :16
            ],  # Firma derivada
        )
