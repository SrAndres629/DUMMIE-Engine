...
from brain.application.use_cases.crystallization import (
    CrystallizeProceduralMemoryUseCase,
)
...
from layers.l2_brain.domain.memory.ports import (
    IEventStorePort,
    ILedgerAuditPort,
    IShieldOutputPort,
...
from layers.l2_brain.domain.memory.models import MemoryNode4DTES, EgoState
...
class CognitiveOrchestrator(IBrainOrchestrator):
    ...
    def __init__(
        ...
        self.crystallize_use_case = CrystallizeProceduralMemoryUseCase(
            event_store=event_store, skill_repo=skill_repo, ledger_audit=ledger_audit
        )
        self.lessons_use_case = CrystallizeLessonsUseCase(ledger_audit=ledger_audit)
    ...
    async def handle_task(self, payload: Union[str, AgentIntent]) -> str:
        ...
        try:
            ...
            if (
                stats.certainty_score < 0.5
                and intent.intent_i == ContextIntent.MUTATION
            ):
                raise Exception(
                    f"[Spec 42] Ontological lock: Certeza insuficiente ({stats.certainty_score}) para mutación en {intent.locus_x}"
                )
            ...
            node = MemoryNode4DTES(
                ...
                authority_a=str(intent.authority_a.value)
                if hasattr(intent.authority_a, "value")
                else str(intent.authority_a),
                intent_i=str(intent.intent_i.value)
                if hasattr(intent.intent_i, "value")
                else str(intent.intent_i),
            ...
            if self.embedding_port:
                node.embedding = await self.embedding_port.generate_embedding_async(
                    intent.rationale
                )
            ...
            self.ledger_audit.record_decision(
                DecisionRecord(
                    ...
                    witness_hash=hashlib.sha256(
                        node.causal_hash.encode()
                    ).hexdigest(),  # Mock Sentinel
                    metadata={"impact_details": impact},
                )
            )

            # 7. Registro en Session Ledger (Spec 36)
            self.session_ledger.record_ego_state(
                EgoState(
            ...
            self.lessons_use_case.execute_error(
                context=context_fallback, error=e, tick=self.lamport_clock
            )
            raise e
