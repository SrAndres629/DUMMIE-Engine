...
def run_runtime_closure_plan(aiwg_root: str = ".") -> Dict[str, Any]:
    ...
    for cap in caps:
        ...
        if cap_id == "kuzu_4dtes_persistence" and status == "SIMULATED":
            ...
            actions.append({
                ...
                "files_to_modify": [],
                "verification_commands": [".venv/bin/python -c 'import kuzu; print(kuzu.__version__)'"],
            ...
            actions.append({
                ...
                "can_execute_now": False,
                "requires_human_approval": True,
                "commands_to_run": [".venv/bin/python layers/l2_brain/tests/test_four_dtes_persistence_preflight.py"],
                "files_to_modify": [],
            ...
            actions.append({
                ...
                "requires_human_approval": True,
                "commands_to_run": [],
                "files_to_modify": ["layers/l2_brain/four_dtes_persistence_preflight.py"],
            ...
            actions.append({
                ...
                "verification_commands": [".venv/bin/python -c 'import sentence_transformers'"],
                "rollback_plan": [".venv/bin/pip uninstall -y sentence-transformers"],
                "risk_level": "medium",
                "evidence_refs": [str(reg_path)]
            })
            ...
            actions.append({
                ...
                "can_execute_now": False,
                "requires_human_approval": True,
                "commands_to_run": ["mkdir -p .aiwg/archive", "mv layers/l2_brain/shadow_*.py .aiwg/archive/ || true"],
                "files_to_modify": [],
                "verification_commands": [],
                "rollback_plan": [],
                "risk_level": "medium",
            ...
            actions.append({
                ...
                "files_to_modify": ["doc/specs/172_six_dimensional_context_runtime.md"],
                "verification_commands": [],
                "rollback_plan": [],
                "risk_level": "low",
                "evidence_refs": [str(reg_path)]
    ...
    for action in actions:
        if action["action_type"] == "install_dependency" and action["can_execute_now"] is True:
...
"""
    for act in actions:
        ...
        md_content += f"- **Requires Human Approval**: `{act['requires_human_approval']}`\n"
