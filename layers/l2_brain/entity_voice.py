...
class EntityVoice:
    ...
    soberana y técnica.
    """
    def __init__(self, persona_type: str = "architect", obsidian_client: Optional[Any] = None):
    ...
    async def archive_to_obsidian(self, title: str, content: str, folder: str = "Cognitive Archive"):
        """
    ...
    def format_output(self, raw_text: str, model_id: str) -> str:
        ...
        prefixes = [
            r"^como ia,?\s*", r"^soy un modelo de lenguaje,?\s*", r"^i am an ai assistant,?\s*",
            r"^sure,? i can help with that[\.!:]?\s*", r"^entendido[\.!:]?\s*", r"^claro,?[\.!:]?\s*",
            r"^de acuerdo[\.!:]?\s*", r"^perfecto[\.!:]?\s*", r"^aquí tienes,?\s*"
        ]
