...
class MentalModelHygieneFinding:
    ...
    recommended_status: str  # valid | stale | superseded | quarantined | unsafe_rejected | needs_review
...
_COMPLEX_KEYWORDS = frozenset([
    "autonom", "synthesis", "kuzu", "degrad", "missing", "risk",
    "refactor", "plan", "scale", "scaling", "flywheel",
])

_PLANNING_KEYWORDS = frozenset(["plan", "decide", "what should", "next"])

_UNSAFE_MARKERS = frozenset([
    "secret", "private_chain_of_thought", "credential", "api_key",
...
def run_mental_model_truth_hygiene(aiwg_root: Path = Path(".aiwg")) -> Dict[str, Any]:
    ...
    for m in models:
        ...
        if qs >= 100 and kuzu_degraded:
            finding_list.append(MentalModelHygieneFinding(
                mid, "overconfidence_model",
        ...
        if is_complex and not rels:
            finding_list.append(MentalModelHygieneFinding(
                mid, "empty_relations_for_complex_intent",
        ...
        if is_complex and not asms:
            finding_list.append(MentalModelHygieneFinding(
                mid, "empty_assumptions_for_complex_intent",
        ...
        if is_planning and not decs:
            finding_list.append(MentalModelHygieneFinding(
                mid, "empty_decisions_for_planning_intent",
        ...
        if kuzu_degraded and not conts and is_complex:
            finding_list.append(MentalModelHygieneFinding(
                mid, "empty_contradictions_despite_degraded_evidence",
        ...
        if not evidence:
            finding_list.append(MentalModelHygieneFinding(
                mid, "missing_evidence_refs",
                "No evidence references linked",
                "WARN", "needs_review" if is_complex else "stale"
            ))
            if status == "valid":
        ...
        if not fals and is_complex:
            finding_list.append(MentalModelHygieneFinding(
                mid, "missing_falsification_tests",
        ...
        superseded_by = ""
        if len(peers) > 1:
            best = max(peers, key=lambda x: (x.get("quality_score", -1), len(x.get("relations", []))))
            if best.get("model_id") != mid and m.get("quality_score", -1) < best.get("quality_score", -1):
        ...
        decisions.append(MentalModelHygieneDecision(
            model_id=mid,
            previous_status=old_index.get(mid, {}).get("status", "unknown") if isinstance(old_index.get(mid), dict) else "unknown",
            new_status=status,
        ...
        enriched_index[mid] = {
            "path": str(jsonl_path.relative_to(aiwg_root.parent)) if aiwg_root.parent != Path(".") else str(jsonl_path),
            "status": status,
            "quality_score": qs,
        ...
        if status == "quarantined":
            quarantined.append({"model_id": mid, "intent": intent, "quality_score": qs,
                                "findings": [asdict(f) for f in finding_list]})

        lineage.append({
            "model_id": mid, "intent_hash": ih, "intent": intent[:80],
    ...
    # Counters ---------------------------------------------------------
    counts = {"valid": 0, "stale": 0, "superseded": 0, "quarantined": 0, "unsafe_rejected": 0, "needs_review": 0}
    for entry in enriched_index.values():
        s = entry["status"]
        if s in counts:
            counts[s] += 1

    overconfidence_count = sum(1 for f in findings if f.finding_type == "overconfidence_model")
    ...
    (models_root / "runtime_model_hygiene.json").write_text(json.dumps(result.to_dict(), indent=2), encoding="utf-8")
    (models_root / "runtime_model_quarantine.json").write_text(json.dumps(quarantined, indent=2), encoding="utf-8")
    (models_root / "runtime_model_lineage.json").write_text(json.dumps(lineage, indent=2), encoding="utf-8")
    (reports_root / "mental_model_truth_hygiene_latest.json").write_text(json.dumps(result.to_dict(), indent=2), encoding="utf-8")
