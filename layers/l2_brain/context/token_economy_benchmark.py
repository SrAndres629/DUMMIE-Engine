...
class TokenEconomyBenchmark:
    ...
    def run_benchmark(self) -> TokenEconomyBenchmarkReport:
        ...
        if cases[0].estimated_input_tokens > 0:
            reduction_ratio = cases[0].estimated_input_tokens / max(
                1, cases[-1].estimated_input_tokens
            )
    ...
    def _save_report(self, report: TokenEconomyBenchmarkReport):
        ...
        (self.reports_root / "token_economy_benchmark_latest.md").write_text(
            md, encoding="utf-8"
        )
...
def run_token_economy_benchmark(
