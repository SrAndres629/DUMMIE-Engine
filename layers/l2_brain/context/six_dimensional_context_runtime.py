...
def build_6d_context_packet(intent: str, aiwg_root: Path = None) -> dict:
    ...
    if scanner_path.exists():
        ...
        try:
            ...
            if age_hours > 24.0:
                stale_found = True
                warnings.append(
                    f"whole_body_scan_latest.json is stale (age: {age_hours:.1f}h)"
                )
    ...
    axes = {
        ...
        SixDContextAxis.CAUSAL: {
            ...
            "anomalies_detected": len(
                wiring_data.get("anomaly_summary", {}).get("source_without_tests", [])
            ),
        },
        SixDContextAxis.AUTHORITY_SAFETY: {
            "daily_readiness": readiness_data.get("calibrated_scores", {}).get(
                "daily_use_readiness", 0.0
            ),
            "autonomy_authorized": readiness_data.get("calibrated_scores", {}).get(
        ...
        SixDContextAxis.RESOURCE: {
            "total_tokens_consumed": token_data.get("benchmark_results", {}).get(
    ...
    if estimated_tokens > budget:
        warnings.append(
            f"Estimated tokens ({estimated_tokens}) exceeds budget ({budget}). Pruning context items."
        )
    ...
    packet = {
        ...
        "items": items,
        "must_preserve": [
            item["path"] for item in items if item.get("coherence", 0.0) > 80.0
        ],
        "should_include": [
            item["path"] for item in items if item.get("coherence", 0.0) <= 80.0
        ],
    ...
    for item in items:
        md_lines.append(
            f"- **[{item['path']}](file://{aiwg_root}/{item['path']})** (Status: `{item['status']}`, Coherence: {item['coherence']:.1f}%)"
        )
