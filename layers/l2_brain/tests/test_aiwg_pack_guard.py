...
def test_current_truth_mandatory_fields():
    ...
    mandatory_keys = [
        ...
        "unknown_count",
        "orphan_test_candidate_count",
        "degraded_embeddings",
        "real_text_fast_embeddings",
        "vector_spaces_used",
        "active_capabilities",
...
def test_active_pack_rollback_and_tests():
    ...
    assert "rollback_plan" in active or "rollback" in active, (
        "active_pack.json missing rollback plan"
    )
    assert "tests_required" in active or "tests" in active, (
        "active_pack.json missing tests_required"
    )
    ...
    assert isinstance(rollback, str) and len(rollback.strip()) > 0, (
        "Rollback plan must be a non-empty string"
    )
    assert isinstance(tests, list) and len(tests) > 0, (
        "tests_required must be a non-empty list"
    )
...
def test_self_critique_fails_if_unverified(tmp_path, monkeypatch):
    ...
    assert excinfo.value.code != 0, (
        "self-critique must exit with non-zero code if any field is UNVERIFIED"
    )
...
def test_closeout_fails_if_missing_evidence(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        guard, "EVIDENCE_JSON", os.path.join(tmp_path, "missing_evidence.json")
    )

    with pytest.raises(SystemExit) as excinfo:
        guard.run_closeout(None)
    assert excinfo.value.code != 0, (
        "closeout should fail when validation evidence is missing"
    )
...
def test_closeout_fails_if_invalid_roadmap_sha(tmp_path, monkeypatch):
    ...
    with open(temp_truth, "w", encoding="utf-8") as f:
        json.dump({"current_pack": "AIWG_KERNEL_0.1", "head_commit": "actual_hash"}, f)
    ...
    monkeypatch.setattr(
        guard, "DECISION_LOG", os.path.join(tmp_path, "decision_log.jsonl")
    )
    ...
    with pytest.raises(SystemExit) as excinfo:
        guard.run_closeout(None)
    assert excinfo.value.code != 0, (
        "closeout should fail if roadmap contains invalid source of truth commit format"
    )
...
def test_distance_report_contains_pack_6_1():
    ...
    assert "golden_path_remaining" in dist or "current_score" in dist, (
    ...
    with open(md_path, "r", encoding="utf-8") as f:
        md_content = f.read()
    assert "Pack 6.1" in md_content or "6.1" in md_content, (
        "Distance markdown must reference target Pack 6.1"
    )
...
def test_next_pack_skip_prevention(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(guard, "STATE_TRUTH", temp_truth)
    monkeypatch.setattr(
        guard, "ROADMAP_JSON", os.path.join(tmp_path, "missing_roadmap.json")
    )

    with pytest.raises(SystemExit) as excinfo:
        guard.run_next_pack(None)
    assert excinfo.value.code != 0, (
        "next-pack should fail if we attempt to skip PACK_3.1"
    )
...
def test_run_required_failure(tmp_path, monkeypatch):
    ...
    temp_active = os.path.join(tmp_path, "active_pack.json")
    with open(temp_active, "w", encoding="utf-8") as f:
        json.dump({"pack_id": "TEST_RUNNER_PACK", "commands_required": ["false"]}, f)
...
def test_closeout_fails_if_non_runner_evidence(tmp_path, monkeypatch):
    ...
    temp_evidence = os.path.join(tmp_path, "evidence.json")
    with open(temp_evidence, "w", encoding="utf-8") as f:
        json.dump({"exit_code": 0, "commit": "actual_hash"}, f)
...
def test_freshness_gate_stale_roadmap(tmp_path, monkeypatch):
    ...
    temp_roadmap = os.path.join(tmp_path, "roadmap.json")
    with open(temp_roadmap, "w", encoding="utf-8") as f:
        json.dump({"head_commit": "stale_roadmap_hash", "packs": []}, f)
...
def test_anti_overclaim_lint_rejection(tmp_path, monkeypatch):
    ...
    with open(bad_pack, "w", encoding="utf-8") as f:
        f.write(
            '{"claim": "This engine has 100% coverage and is perfecto with sin deuda"}'
        )
...
def test_anti_overclaim_lint_ignores_generated_health_cron_progress(
    tmp_path, monkeypatch
):
    """Pytest progress in generated health cron output is not a governance claim."""
    import scripts.aiwg_pack_guard as guard

    state_dir = os.path.join(tmp_path, "state")
    os.makedirs(state_dir, exist_ok=True)

    health_log = os.path.join(state_dir, "health_cron_latest.txt")
...
def test_aiwg_kernel_freeze_consistency():
    """Assert that AIWG governance state is internally consistent."""
    ...
    # 3. Assert current runtime pack consistency
    assert truth.get("current_pack") == active.get("pack_id"), (
        "current_truth and active_pack must agree on current runtime pack"
    )

    swarm_state = truth.get("swarm_state", {})
    if swarm_state:
        assert truth.get("next_pack") == swarm_state.get("pending_pack")
        assert truth.get("last_completed_pack") == swarm_state.get(
            "latest_completed_pack"
        )

    # 5. Assert freeze audit report values
    audit_json = os.path.join(
        AIWG_DIR, "reports", "aiwg_kernel_freeze_audit_latest.json"
    )
    assert os.path.exists(audit_json), "aiwg_kernel_freeze_audit_latest.json must exist"
    with open(audit_json, "r", encoding="utf-8") as f:
        audit = json.load(f)
    assert audit.get("decision") == "KERNEL_FROZEN", (
        "freeze_audit decision must be KERNEL_FROZEN"
    )
    assert audit.get("head_commit") == truth.get("head_commit"), (
        "freeze_audit head_commit must match current_truth head_commit"
    )

    # 6. Assert validation evidence values
    assert os.path.exists(EVIDENCE_JSON), (
        "pack_validation_evidence_latest.json must exist"
    )
    ...
    assert guard.check_anti_overclaim_lint() is True, (
        "anti-overclaim lint scanner must pass cleanly"
    )
