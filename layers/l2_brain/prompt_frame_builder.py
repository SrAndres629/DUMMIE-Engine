...
class PromptFrameBuilder:
    ...
    def build_prompt_frame_for_phase(
        ...
        # Security checks
        for ref in system_refs + mission_refs + truth_policy_refs + context_refs + compressed_refs + dropped_refs + retrieval_refs:
    ...
    def _extract_staleness_warnings(self, stale_report: dict[str, Any]) -> list[str]:
        findings = stale_report.get("findings", []) if isinstance(stale_report, dict) else []
        warnings = []
    ...
    def _ensure_not_raw_dump_ref(self, ref: str) -> None:
        ...
        if not candidate:
            return
        normalized = candidate.rstrip("/") + "/" if candidate.endswith("/") else candidate
