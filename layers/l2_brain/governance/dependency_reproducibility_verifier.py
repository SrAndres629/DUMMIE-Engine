...
def run_dependency_reproducibility_verification() -> dict:
    ...
    heavy_dependencies = []
    warnings = []
    evidence_refs = ["layers/l2_brain/pyproject.toml", "layers/l2_brain/.venv"]
    ...
    has_undeclared_heavy = any(
        x in undeclared_installed_packages for x in ["sentence_transformers", "torch"]
    )
    ...
    if has_undeclared_heavy:
        ...
        warnings.append(
            f"Undeclared installed packages: {undeclared_installed_packages}"
        )
    else:
        decision = "PASS"
