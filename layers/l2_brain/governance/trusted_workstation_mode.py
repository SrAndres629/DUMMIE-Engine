...
class TrustedWorkstationMode:
    OBSOLETE_CATEGORIES = {
        "READ_ONLY_STATUS",
        "READ_ONLY_FILE_METADATA",
        "READ_ONLY_REPO_INSPECTION",
    }
    COGNITIVE_CATEGORIES = {
        "ANALYZE_PLAN",
        "SPEC_AUTHORING",
        "TEST_COMMAND_RECOMMENDATION",
        "PATCH_PROPOSAL",
    }
    VERIFIED_MUTATION_CATEGORIES = {
        "WORKSPACE_WRITE",
        "WORKSPACE_EDIT",
        "TEST_EXECUTION",
        "TEST_RUN",
    ...
    def evaluate_action(self, action: WorkstationAction) -> WorkstationDryRunResult:
        ...
        if (
            action.category in self.COGNITIVE_CATEGORIES
            and not action.requires_workspace_mutation
        ):
            return WorkstationDryRunResult(
                action_id=action.action_id,
                decision="ALLOW",
        ...
        if (
            action.requires_workspace_mutation
            or action.category in self.VERIFIED_MUTATION_CATEGORIES
        ):
    ...
    def run_mode_report(self) -> dict[str, Any]:
        samples = [
            ...
            WorkstationAction(
                ...
                "WORKSPACE_WRITE",
                "write file",
                evidence_refs=["python3 scripts/verify_before_commit.py"],
...
def evaluate_workstation_action(
    action: WorkstationAction, aiwg_root: str | Path = ".aiwg"
) -> WorkstationDryRunResult:
