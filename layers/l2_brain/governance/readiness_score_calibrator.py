...
class ReadinessScoreCalibrator:
    ...
    def run_calibration(self) -> ReadinessCalibrationReport:
        ...
        missing_tests = [
            m
            for m in metrics
            if m.get("category") == "TEST_PRESENCE" and m.get("status") == "MISSING"
        ]
        if missing_tests:
        ...
        if not chat_cli.get("memory_spine") and not chat_cli.get(
            "memory_spine_used", False
        ):
            findings.append(
                ReadinessCalibrationFinding(
        ...
        memory_penalty = sum(
            f.score_penalty for f in findings if "memory" in f.id or "kuzu" in f.id
        )
        entrypoint_penalty = sum(
            f.score_penalty for f in findings if "entrypoint" in f.id
        )
        token_penalty = sum(f.score_penalty for f in findings if "token" in f.id)
        advisory_penalty = sum(
            f.score_penalty for f in findings if "advisory" in f.id or "dry_run" in f.id
        )
        ...
        autonomy_score = max(
            0.0, base - total_penalty - 2.0
        )  # Extra penalty for autonomous mutation
    ...
    def _save_report(self, report: ReadinessCalibrationReport) -> None:
        ...
        (self.reports_root / "readiness_score_calibration_latest.md").write_text(
            md, encoding="utf-8"
        )
...
def run_readiness_score_calibration(
