...
class RepoProbeRunner:
    ...
    def run_all_probes(self) -> RepoProbeResult:
        ...
        try:
            ...
            return RepoProbeResult(
                ...
                findings=[
                    RepoProbeFinding(
                        "git_ls_files", "ERROR", f"Failed to list files: {exc}"
                    )
                ],
    ...
    def _probe_layers(
        self, files: list[str], findings: list[RepoProbeFinding]
    ) -> dict[str, Any]:
        layer_map = {
            ...
            if count == 0:
                findings.append(
                    RepoProbeFinding(
                        "layer_presence",
                        "WARNING",
                        f"Layer {layer} appears empty or missing",
                        {"path": path},
                    )
                )
        ...
        if ratios.get("Python", 0) > 0.8:
            findings.append(
                RepoProbeFinding(
                    "polyglot_check",
                    "WARNING",
                    "High Python concentration detected. Ensure polyglot compliance.",
            ...
            if not exists:
                findings.append(
                    RepoProbeFinding(
                        "runtime_module",
                        "ERROR",
                        f"Critical runtime module missing: {path}",
                    )
                )

        return status

    def _probe_specs_and_tests(
        self, files: list[str], findings: list[RepoProbeFinding]
    ) -> None:
        specs = [f for f in files if f.startswith("doc/specs/") and f.endswith(".md")]
        features = [
            f for f in files if f.startswith("doc/specs/") and f.endswith(".feature")
        ]
        rules = [
            f for f in files if f.startswith("doc/specs/") and f.endswith(".rules.json")
        ]

        if len(specs) == 0:
            findings.append(
                RepoProbeFinding(
                    "spec_presence", "WARNING", "No Markdown specs found in doc/specs/"
                )
            )
        ...
        for spec in specs:
            ...
            if feat not in features or rule not in rules:
                findings.append(
                    RepoProbeFinding(
                        "spec_triplet", "WARNING", f"Incomplete spec triplet for {spec}"
                    )
                )
        ...
        if len(tests) == 0:
            findings.append(
                RepoProbeFinding(
                    "test_presence", "WARNING", "No tests found in the repository"
                )
            )
    ...
    def _probe_state_coherence(self, findings: list[RepoProbeFinding]) -> None:
        ...
        if not guard_report.exists():
            findings.append(
                RepoProbeFinding(
                    "state_coherence", "WARNING", "State coherence guard report missing"
                )
            )
            return
        ...
        try:
            ...
            if data.get("decision") == "FAIL":
                findings.append(
                    RepoProbeFinding(
                        "state_coherence",
                        "ERROR",
                        "Latest state coherence guard failed",
                        data.get("findings"),
                    )
                )
        except Exception as exc:
            findings.append(
                RepoProbeFinding(
                    "state_coherence",
                    "ERROR",
                    f"Failed to read state coherence report: {exc}",
                )
            )
