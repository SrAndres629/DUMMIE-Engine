...
class DebateReviewRuntime:
    ...
    def run_debate(self) -> DebateReviewResult:
        ...
        swarm_output = self._load_json(
            self.reports_root / "strategic_partner_swarm_latest.json"
        )
        coherence = self._load_json(
            self.reports_root / "mission_coherence_guard_latest.json"
        )
        ...
        evidence_gaps = []
        if not mission_plan:
            evidence_gaps.append("missing_mission_plan")
        if not swarm_output:
            evidence_gaps.append("missing_swarm_output")
        ...
        res = DebateReviewResult(
            ...
            judge_verdict={
                ...
                "allowed_to_continue": decision
                in ["accept_plan", "accept_with_objections"],
            },
    ...
    def _opinion_evidence_auditor(self, coherence: dict) -> DebateRoleOpinion:
        ...
        if coherence.get("decision") != "PASS":
            obj.append(
                "physical_evidence_mismatch: mission artifacts not coherent with roadmap"
            )
        return DebateRoleOpinion(
    ...
    def _opinion_risk_challenger(self, plan: dict, swarm: dict) -> DebateRoleOpinion:
        ...
        return DebateRoleOpinion(
            role="risk_challenger", claim="Auditing operational risks", risk_flags=risks
        )
    ...
    def _opinion_implementation_reviewer(self, plan: dict) -> DebateRoleOpinion:
        ...
        for l3 in plan.get("l3_microphases", []):
            if not l3.get("tests_to_run"):
                obj.append(
                    f"missing_tests: microphase {l3.get('microphase_id')} lacks test paths"
                )
        return DebateRoleOpinion(
    ...
    def _opinion_mentor_judge(self, coherence: dict, swarm: dict) -> DebateRoleOpinion:
        if coherence.get("decision") == "FAIL":
            return DebateRoleOpinion(
                role="mentor_judge",
                claim="Blocking due to coherence failure",
                recommended_action="block",
            )
        if swarm.get("decision") == "block_due_to_coherence_failure":
            return DebateRoleOpinion(
                role="mentor_judge",
                claim="Swarm blocked execution",
                recommended_action="block",
            )
        ...
        return DebateRoleOpinion(
            ...
            recommended_action="accept_with_objections"
            if swarm.get("risk_flags")
            else "accept_plan",
        )

    def _resolve_decision(self, judge: DebateRoleOpinion, coherence: dict) -> str:
        if judge.recommended_action == "block":
            return "block"
        if coherence.get("decision") == "FAIL":
            return "repair_before_next_phase"
