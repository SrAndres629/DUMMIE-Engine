...
from layers.l2_brain.autonomous_strategic_partner_runtime import (
    run_autonomous_strategic_partner_runtime,
)
...
class CliControlPlane:
    ...
    def run_command(self, command: str) -> CliCommandResult:
        ...
        handlers = {
            ...
            "cache-summary": lambda: self._read_latest(
                "prompt_cache_summary_latest.json"
            ),
            "restart-gate": lambda: self._read_latest(
                "restart_integration_gate_latest.json"
            ),
            "benchmark": lambda: self._read_latest(
                "context_efficiency_benchmark_latest.json"
            ),
            ...
            "capability-scorecard": lambda: self._read_latest(
                "plan_v1_runtime_capability_scorecard.json"
            ),
            "integration-backlog": lambda: self._read_latest(
                "integration_backlog.json"
            ),
        ...
        if command not in handlers:
            result = CliCommandResult(
                ...
                payload={
                    ...
                    "available_commands": sorted(handlers),
                },
    ...
    def _cmd_next_action(self) -> CliCommandResult:
        ...
        next_seed = self._load_json(
            self.aiwg_root / "evolution" / "next_phase_seed.json"
        )

        action = flywheel.get("decision", "review_runtime_outputs")
        recommended_phase = flywheel.get(
            "recommended_next_phase", next_seed.get("next_phase", "unknown")
        )
        payload = {
            "decision": action,
    ...
    def _cmd_mission_dag(self) -> CliCommandResult:
        ...
        try:
            ...
            plan.l3_microphases = [
                type("MicroPhase", (), m) for m in plan_data["l3_microphases"]
            ]
    ...
    def _cmd_strategic_swarm(self) -> CliCommandResult:
        ...
        return CliCommandResult(
            ...
            decision="PASS"
            if decision.decision == "continue_next_phase"
            else "PASS_WITH_WARNINGS"
            if "repair" in decision.decision
            else "FAIL",
    ...
    def _cmd_debate_review(self) -> CliCommandResult:
        ...
        return CliCommandResult(
            ...
            decision="PASS"
            if result.decision in ["accept_plan", "accept_with_objections"]
            else "FAIL",
            payload=result.to_dict(),
    ...
    def _cmd_plan_v1_status(self) -> CliCommandResult:
        ...
        return CliCommandResult(
            ...
            decision="PASS"
            if decision.plan_v1_completion_status == "complete"
            else "PASS_WITH_WARNINGS",
    ...
    def _cmd_repo_intelligence(self) -> CliCommandResult:
        res = run_repo_intelligence_scan(
            repo_root=self.aiwg_root.parent, aiwg_root=self.aiwg_root.name
        )
        return CliCommandResult(
    ...
    def _cmd_entrypoint_audit(self) -> CliCommandResult:
        ...
        return CliCommandResult(
            ...
            warnings=[
                a["entrypoint"]
                for a in res.get("audits", [])
                if not a.get("uses_memory_spine")
            ],
    ...
    def _cmd_dialectical_review(self) -> CliCommandResult:
        res = run_metacognitive_loop(
            "challenge my current state", aiwg_root=self.aiwg_root
        )
    ...
    def _cmd_metacognitive_flywheel(self) -> CliCommandResult:
        ...
        return CliCommandResult(
            "metacognitive-flywheel",
            "PASS",
            delta,
            [],
            [".aiwg/reports/metacognitive_evolution_delta_latest.json"],
            self._utc_now(),
        )
    ...
    def _cmd_six_d_context(self) -> CliCommandResult:
        ...
        res = build_6d_context_packet(
            intent="repair kuzu", aiwg_root=self.aiwg_root.parent
        )
        return CliCommandResult(
            "6d-context",
            res.get("decision", "PASS"),
    ...
    def _cmd_context_optimize(self) -> CliCommandResult:
        ...
        packet = build_6d_context_packet(
            intent="repair kuzu", aiwg_root=self.aiwg_root.parent
        )
    ...
    def _cmd_embedding_router(self) -> CliCommandResult:
        ...
        res = run_embedding_memory_router_demo(
            intent="repair kuzu", aiwg_root=self.aiwg_root.parent
        )
        return CliCommandResult(
            "embedding-router",
    ...
    def _cmd_daemon_gateway_bridge(self) -> CliCommandResult:
        ...
        res = run_daemon_gateway_bridge_demo(
            intent="repair kuzu", aiwg_root=self.aiwg_root.parent
        )
        return CliCommandResult(
            "daemon-gateway-bridge",
            res.get("decision", "PASS"),
            res,
            evidence_refs=[
