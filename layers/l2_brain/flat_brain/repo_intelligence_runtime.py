...
class RepoIntelligenceRuntime:
    ...
    def _classify_file(self, path: str) -> RepoFileFact:
        ...
        # language
        lang_map = {".py": "python", ".js": "javascript", ".ts": "typescript", ".md": "markdown", ".json": "json", ".yaml": "yaml", ".yml": "yaml", ".sh": "bash"}
        ...
        is_runtime = language in ["python", "bash", "typescript"] and not is_test and not is_spec and "layers/" in path
        
        is_generated_or_vendor = "node_modules/" in path or "__pycache__" in path or ".venv" in path

        artifact_type = "unknown"
        if is_test: artifact_type = "test"
        elif is_spec: artifact_type = "spec"
        elif is_schema: artifact_type = "schema"
        elif is_report: artifact_type = "report"
        elif is_runtime: artifact_type = "runtime"

        category = "first_party"
        if is_generated_or_vendor: category = "dependency"
        elif is_report: category = "generated"
...
def run_repo_intelligence_scan(repo_root: str | Path = ".", aiwg_root: str | Path = ".aiwg") -> dict[str, Any]:
