...
def _process_check(name: str, layer: str, needle: str, next_repair: str = "") -> TruthCheck:
    command = "ps -eo cmd"
    try:
        out = subprocess.check_output(["ps", "-eo", "cmd"], text=True, timeout=2)
    except Exception as exc:
        return TruthCheck(name, layer, TruthStatus.UNKNOWN, command=command, error=str(exc))

    if needle in out:
        return TruthCheck(name, layer, TruthStatus.PASS, [f"process contains {needle}"], command=command)
    return TruthCheck(
        name,
        layer,
...
def _model_discovery_check(include_slow: bool) -> TruthCheck:
    ...
    try:
        ...
        return TruthCheck("l2.model_discovery.live", "L2", TruthStatus.PASS, [str(counts)])
...
def collect_truth(repo_root: str, include_slow: bool = False) -> TruthReport:
    ...
    checks: list[TruthCheck] = [
        ...
        _file_check(repo, "l3.compliance_auditor.file", "L3", "layers/l3_shield/compliance_auditor.py"),
        _file_check(repo, "l5.mcp_driver.file", "L5", "layers/l5_muscle/mcp_driver.py"),
        _file_check(repo, "l6.skin.package", "L6", "layers/l6_skin/package.json", degraded_if_missing=True),
        _jsonl_check(repo, "l1.swarm_ledger.path", "L1", ".aiwg/memory/swarm_ledger.jsonl"),
        _jsonl_check(repo, "l2.token_ledger.path", "L2", ".aiwg/ledger/token_usage.jsonl"),
        _jsonl_check(repo, "root.sovereign_ledger.path", "ROOT", "ledger/sovereign_resolutions.jsonl"),
        _import_check("l1.gateway.import", "L1", "mcp_server", "repair L1 import path or dependencies"),
