...
class RepoIntelligenceQueryRuntime:
    ...
    def query_repo_intelligence(self, query: dict[str, Any]) -> RepoIntelligenceQueryResult:
        inventory_path = self.intel_root / "repo_inventory.json"
        if not inventory_path.exists():
            return RepoIntelligenceQueryResult(query=query, results=[], count=0, decision="FAIL", generated_at=self._utc_now())
        ...
        for f in files:
            if layer and f.get("layer") != layer: continue
            if language and f.get("language") != language: continue
            if is_runtime is not None and f.get("is_runtime") != is_runtime: continue
            if is_test is not None and f.get("is_test") != is_test: continue
            if is_spec is not None and f.get("is_spec") != is_spec: continue
            if category and f.get("category") != category: continue
            if path_contains and path_contains not in f["path"]: continue
...
def query_repo_intelligence(query: dict[str, Any], aiwg_root: str | Path = ".aiwg") -> RepoIntelligenceQueryResult:
