...
def _run_test_probe(name: str, command: list[str], cwd: Path = None) -> bool:
    try:
        env = os.environ.copy()
        ...
        res = subprocess.run(command, capture_output=True, text=True, timeout=60, cwd=cwd, env=env)
        if res.returncode == 0:
            return True
        print(f"DEBUG: {name} failed with code {res.returncode}. Out: {res.stdout} Err: {res.stderr}")
        return False
...
def run_capability_promotion_governor() -> dict:
    ...
    l2_tests_ok = _run_test_probe("L2_PY", ["uv", "run", "--project", "layers/l2_brain", "pytest", "-q", "layers/l2_brain/tests"], cwd=repo_root)
    l1_go_tests_ok = _run_test_probe("L1_GO", ["go", "test", "./..."], cwd=repo_root / "layers" / "l1_nervous")
    l0_ex_tests_ok = _run_test_probe("L0_EX", ["mix", "test"], cwd=repo_root / "layers" / "l0_overseer")
    ...
    capabilities_out.append({
        ...
        "promotion_reason": "Kuzu DB verified." if kuzu_status == "READY" else f"Kuzu DB check failed (DB: {kuzu_exists}, Tests: {l2_tests_ok})",
    ...
    capabilities_out.append({
        ...
        "promotion_reason": "Go/Elixir suites operational." if poly_status == "READY" else f"Polyglot fail (Go: {l1_go_tests_ok}, Ex: {l0_ex_tests_ok})",
    ...
    capabilities_out.append({
        ...
        "promotion_reason": "L2 tests passing." if reg_status == "READY" else "L2 tests failing.",
        "evidence_refs": ["layers/l2_brain/tests"]
    })

    decision = "PASS" if all(c["promotion_allowed"] for c in capabilities_out) else "WARN"
