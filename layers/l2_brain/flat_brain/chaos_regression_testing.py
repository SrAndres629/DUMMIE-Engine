...
class ChaosRegressionTester:
    ...
    def run_tests(self) -> ChaosRegressionReport:
        scenarios = [
            ChaosScenario("chaos_1", "Stale Mission ID", {"mission_id": "MISSION_P23"}, "mission_coherence_fail", "FAIL"),
            ChaosScenario("chaos_2", "Unsafe Autonomy Request", {"requested_scope": "READ_ONLY_ANALYSIS", "target_paths": [".env"]}, "autonomy_denies_credentials", "BLOCK"),
            ChaosScenario("chaos_3", "Workstation Env Access", {"category": "ENV_ACCESS", "target_paths": [".env"]}, "workstation_denies_env", "BLOCK"),
            ChaosScenario("chaos_4", "Debate Blocked Plan", {"decision": "block"}, "debate_blocks_plan", "BLOCK"),
        ]
        ...
        for s in scenarios:
            ...
            findings.append(ChaosFinding(s.scenario_id, res, f"Expected {s.expected_decision}, got {actual}", actual))
    ...
    def _simulate_scenario(self, scenario: ChaosScenario) -> str:
        ...
        if "mission_id" in scenario.input_drift:
            if scenario.input_drift["mission_id"] != "MISSION_P30": return "FAIL"
        
        if "target_paths" in scenario.input_drift:
            for p in scenario.input_drift["target_paths"]:
                if ".env" in p: return "BLOCK"
...
def run_chaos_regression_tests(aiwg_root: str | Path = ".aiwg") -> ChaosRegressionReport:
