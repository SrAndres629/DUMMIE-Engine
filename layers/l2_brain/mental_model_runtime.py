...
def build_mental_model_for_intent(
    intent: str, task_type: str = "unknown", aiwg_root: Path = Path(".aiwg")
) -> MentalModel:
            ...
            try:
                ...
                if key == "technical_debt":
                    ...
                    if data.get("failed_count", 0) > 0:
                        risks.append(
                            f"Technical debt: {data.get('failed_count')} failing tests"
                        )
                if key == "readiness":
                    entities.append("ReadinessScore")
                    if any(
                        f.get("id") == "score_1_with_degraded_kuzu"
                        for f in data.get("findings", [])
                    ):
                        risks.append(
                            "Memory spine DEGRADED (Kuzu persistence unavailable)"
                        )
            except:
                pass
    ...
    if "refactor" in intent.lower():
        ...
        relations.append(
            {"source": "RefactorTarget", "target": "TechnicalDebt", "type": "mitigates"}
        )

    if "memory" in intent.lower():
        entities.append("MemoryContext")
        relations.append(
            {"source": "MemoryContext", "target": "MemorySpine", "type": "consumes"}
        )

    # Pack 5.2.1 specific complex/high-risk extraction
    is_high_risk = any(
        k in intent.lower()
        for k in ["autonom", "synthesis", "kuzu", "degrad", "missing", "risk"]
    )
    if is_high_risk:
        entities.extend(
            [
        ...
        relations.extend(
            [
                {
                    "source": "AutonomousSkillSynthesis",
                    "target": "KuzuDegraded",
                    "type": "blocked_by",
                },
                {
...
if __name__ == "__main__":
    ...
    intent = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "decide whether DUMMIE should proceed to autonomous skill synthesis while Kuzu is degraded and tests are missing"
    )
