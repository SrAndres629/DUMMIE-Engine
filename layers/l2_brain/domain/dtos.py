...
class EpistemicPayload(BaseModel):
    ...
    content: str = Field(
        ..., description="El contenido textual o representativo de la memoria."
    )
    pi_confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Probabilidad/Confianza a priori."
    )
    rho_risk: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Evaluación del riesgo."
    )
    delta_decay: float = Field(
        default=0.0, description="Decaimiento temporal acumulado."
    )
    evidence_hashes: List[str] = Field(
        default_factory=list, description="Soporte evidencial (Enlaces Merkle)."
    )
