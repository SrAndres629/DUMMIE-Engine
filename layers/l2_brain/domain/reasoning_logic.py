...
class ReasoningLogic:
    ...
    @staticmethod
    def rank_candidates(
        goal: str, candidates: List[Dict[str, Any]], max_selected: int = 5
    ) -> List[Dict[str, Any]]:
        ranked = []
        ...
        for candidate in candidates:
            ...
            text = " ".join(
                str(candidate.get(k, ""))
                for k in ("id", "target", "text", "description")
                if candidate.get(k)
            )
            overlap = ReasoningLogic._lexical_overlap(
                goal_tokens, ReasoningLogic._tokens(text)
            )
            ...
            score += overlap * 0.5
            if side_effect == "write" and any(
                t in goal_tokens for t in ["persist", "lesson", "log"]
            ):
                score += 0.15
            ...
            normalized["why"] = (
                f"Deterministic rank score {normalized['score']} with risk {normalized['risk']}."
            )
    ...
    def shape_context_packet(
        ...
        selected = [
            str(item.get("id") or item.get("target"))
            for item in ranked
            if item.get("id") or item.get("target")
        ]
        evidence_bundle = []
        for item in ranked:
