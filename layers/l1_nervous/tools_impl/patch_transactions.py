...
def create_patch_transaction(
    ...
    if not store:
        return {
            "error": "DUMMIE_ROOT_REQUIRED",
            "message": "DUMMIE_ROOT env var must be set for session operations.",
        }
...
def get_patch_transaction_status(
    session_id: str, transaction_id: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if artifact_name not in session["artifacts"]:
            return {
                "error": f"Transaction {transaction_id} not found in session {session_id}"
            }
